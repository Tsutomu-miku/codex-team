...
- `codexm switch --force` and `codexm switch --auto --force` try to apply the auth change immediately to the running codexm-managed Desktop session. If that devtools-based refresh path fails, codexm falls back to force-killing the managed Desktop so it cannot keep running on stale auth.
