...
`codexm watch` attaches to the managed Codex Desktop DevTools session and injects a renderer probe that tees bridge-level `mcp-request` / `mcp-notification` / `mcp-response` traffic between the GUI and app server. By default it only observes and prints bridge traffic; pass `--auto-switch` to trigger `switch --auto` when that bridge traffic shows quota exhaustion, such as exhausted `account/rateLimits/*` payloads or `UsageLimitExceeded`. `codexm watch --detach` runs that watcher in the background and writes state to `~/.codex-team/watch-state.json` plus logs to `~/.codex-team/logs/watch.log`; use `codexm watch --status` and `codexm watch --stop` to inspect or stop it. With `--debug`, it prints the normalized bridge `mcp-*` messages plus watch decision logs to stderr.
